...
[I used the `dba.plotHeatmap()` function; 1 line]
...
### PCA
The above scatter plot appears to be hard to interpret. Instead, we could perform a PCA and color points according to data features. 
```{r peaks_clustering_pca, }
...
# Perform PCA
pca_result <- PCA(variables_for_pca, graph = TRUE)
```
